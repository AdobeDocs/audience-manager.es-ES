...
Este documento proporciona información general sobre la administración de la privacidad de datos individuales y las solicitudes de exclusión que puede enviar a [!DNL Audience Manager] a través de [IU de Privacy Service](https://privacyui.cloud.adobe.io/) y el **[!DNL Privacy Service API]**.
...
Al enviar solicitudes de privacidad de datos individuales, puede enviar cualquier [!DNL Audience Manager] identificadores (ID), tal como se describe en la sección **[Identificadores del Audience Manager](data-privacy-ids.md)** , junto con sus respectivos ID de área de nombres (ID de fuentes de datos).
...
Puede enviar solicitudes de acceso a datos individuales a través del [IU de Privacy Service](https://privacyui.cloud.adobe.io) (documentación) [aquí](https://experienceleague.adobe.com/docs/experience-platform/privacy/ui/overview.html?lang=en) o llamando a la API de Privacy Service (documentación) [aquí](https://experienceleague.adobe.com/docs/experience-platform/privacy/api/overview.html?lang=en) y [!DNL API] reference [aquí](https://developer.adobe.com/experience-platform-apis/references/privacy-service/).
...
Adobe entiende su compromiso de cumplir sus solicitudes de privacidad de datos en un plazo de 30 días. Por esa razón, [!DNL Adobe] se compromete a procesar su solicitud de eliminación de datos lo antes posible.

En respuesta a sus solicitudes de eliminación de datos de consumo, [!DNL Audience Manager] elimina los rasgos y segmentos asociados a [!DNL Audience Manager] identificador incluido en la solicitud. Además, los respectivos [!DNL Audience Manager] identificadores de la persona exclusión de la recopilación de datos adicional por [!DNL Audience Manager] y se eliminarán las asignaciones de ID correspondientes.

Al enviar ID declarados, como entre dispositivos [!DNL CRM] ID o [!DNL cookie] ID, en solicitudes de privacidad de datos, [!DNL Audience Manager] realizará la eliminación necesaria en todos los dispositivos vinculados (hasta 100 dispositivos por ID declarado).
...
Consulte nuestra [documentación de la lista de destinos basados en el dispositivo](assets/AAM-Partners-October2019.xlsx) para ver qué [!DNL Audience Manager] los socios de activación admiten desegmentar.
...
Mientras que las solicitudes de acceso y eliminación de datos se gestionan a través de [Privacy Service](https://experienceleague.adobe.com/docs/experience-platform/privacy/home.html?lang=en), las solicitudes de exclusión se admiten actualmente a través de [!DNL DCS API]. Siga leyendo para saber cuál es la exclusión [!DNL API] las llamadas deberían tener el aspecto siguiente.
...
La exclusión a nivel de socio le permite excluir a los usuarios de la recopilación de datos por [!DNL Audience Manager] socios. Puede enviar solicitudes de exclusión a nivel de socio para ID entre dispositivos, como [!DNL CRM] ID y direcciones de correo electrónico con hash.
...
* [!DNL Audience Manager] elimina de la segmentación el elemento excluido [!DNL CRM] ID y el último ID de dispositivo de todos los segmentos;
...
Cuándo [!DNL Audience Manager] recibe una solicitud de exclusión a nivel de socio, el [!DNL JSON] devuelto por el [!DNL DCS] contiene el [código de error 171](../../api/dcs-intro/dcs-api-reference/dcs-error-codes.md#opt-out-error-codes), con el mensaje [!UICONTROL "Encountered opt out tag"], en lugar del [!DNL Audience Manager] ID de usuario.
...
La exclusión a nivel de socio le permite excluir a los usuarios de la recopilación de datos por [!DNL Audience Manager] socios. Puede desactivar la recopilación de datos en un ID de dispositivo determinado para una marca haciendo las siguientes llamadas a la [API de DCS](../../api/dcs-intro/dcs-api-reference/dcs-api-reference-overview.md):
...
## [!DNL Audience Manager] Socios Con Capacidades De Eliminación De La Segmentación {#aam-partners-with-unsegmentation}
...
Teniendo en cuenta que [!DNL Audience Manager] no es la fuente de los datos, existe una función limitada para la corrección de datos en [!DNL Audience Manager]. La corrección podría significar que el consumidor ha solicitado que se le descalifique de un [!UICONTROL trait]/[!UICONTROL segment] o cualificado para el deseado [!UICONTROL trait]/[!UICONTROL segment].

[!DNL Audience Manager] los clientes pueden elegir capturar las señales, rasgos o segmentos relevantes con perfiles de usuario y enviar esta información a través de [ingesta de datos sin conexión](../../integration/sending-audience-data/batch-data-transfer-explained/batch-data-transfer-overview.md) hasta [!DNL Audience Manager]. Tenga en cuenta que el usuario seguirá cumpliendo los requisitos para el original [!UICONTROL trait] y [!UICONTROL segments] si repiten su comportamiento.
