...
[!UICONTROL Profile Merge Rules] Las opciones de le permiten expandir o ajustar el enfoque de la audiencia en audiencias específicas según las necesidades o los objetivos empresariales. Estos casos de uso generales exploran cómo utilizar las opciones disponibles y crear reglas de combinación para la segmentación individual, doméstica y entre dispositivos. [!UICONTROL Profile Merge Rules] trabajar con destinos por lotes y en tiempo real.

>[!TIP]
>
>Para ver definiciones y descripciones de estos elementos [!UICONTROL Merge Rule] configuración, consulte [Opciones definidas de reglas de combinación de perfiles](merge-rule-definitions.md).
...
Para crear una regla que segmente solo los perfiles de dispositivo, seleccione **[!UICONTROL No Cross-Device Profile]** + **[!UICONTROL Device Profile]**.
...
Mediante el uso de **[!UICONTROL No Cross-Device Profile]** + **[!UICONTROL Device Profile]** regla, [!DNL Device 1] y [!DNL Device 3] ambos cumplen los requisitos para el segmento, mientras que el dispositivo 2 se ignora.
...
El equipo de marketing de la tienda puede utilizar el **[!UICONTROL Current Authenticated Profiles]** + **[!UICONTROL No Device Profile]** para dirigirse a John y Jane con ofertas específicas, basadas únicamente en su actividad autenticada.
...
Este caso de uso abarca la administración de la identidad doméstica. Una empresa puede combinar un perfil de un solo dispositivo con el último perfil que se autenticó en ese dispositivo, utilizando **[!UICONTROL Last Authenticated Profiles]** + **[!UICONTROL Device Profile]** regla.

![last-device-profile](assets/last-device-profile.png)

Consideremos un segmento hecho de hogares con ingresos buenos a $100.000/año, que contiene al menos un dispositivo que es un [!DNL iPhone 7] el [!DNL Data Plan B]. Tenemos dos perfiles domésticos (perfiles multidispositivo), cada uno conectado a dos perfiles de dispositivo diferentes. Los rasgos necesarios para calificar para el segmento se distribuyen entre los perfiles del dispositivo y entre dispositivos.
...
El vínculo entre el dispositivo y el perfil del hogar permite al Audience Manager calificar [!DNL Household 2] para el segmento, pero no [!DNL Household 1]. Desde [!DNL Household 2], solo [!DNL Device 3] cumple los requisitos para el segmento. Esta [!UICONTROL Profile Merge Rule] ha permitido al experto en marketing enviar un mensaje de marketing coherente a un dispositivo individual ([!DNL Device 3]) y el hogar en general ([!DNL Household 2]).

![administración doméstica](assets/household-management.png)
...
Este escenario de segmentación solo está disponible para los clientes que han comprado el [!DNL People-Based Destinations] complemento de. Esta regla permite a los especialistas en marketing llegar a los clientes en función de sus propios datos autenticados.

Supongamos que un minorista en línea desea llegar a los clientes existentes a través de plataformas sociales y mostrarles ofertas personalizadas basadas en sus pedidos anteriores. Con [!UICONTROL People-Based Destinations], pueden ingerir direcciones de correo electrónico con hash desde su propia cuenta [!DNL CRM] en Audience Manager, genere segmentos a partir de los datos sin conexión y envíelos a las plataformas sociales en las que desee anunciarse, utilizando ese identificador hash para optimizar el gasto en publicidad.
...
Elección de una [!UICONTROL device graph] opción para un [!UICONTROL Profile Merge] La regla depende de condiciones únicas para las propiedades digitales y los objetivos empresariales. Estas directrices generales pueden ayudarle a comprender cuándo utilizar un tipo de gráfico frente a otro. Tenga en cuenta que debe tener una relación contractual con un gráfico de dispositivos externos para utilizar estas opciones. Consulte la tabla siguiente para obtener instrucciones generales sobre cuándo elegir una opción de gráfico de dispositivos. Para casos de uso específicos, consulte [Casos de uso de Device Graph de enlace de perfil](profile-link-use-case.md) y [Casos de uso de gráficos de dispositivos externos](external-graph-use-cases.md).
...
<table id="table_66D9152D4FF040A186003272D456625D"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p><span class="wintitle"> Gráfico de dispositivos de enlace de perfil</span> </p> </td> 
   <td colname="col2"> <p><span class="wintitle"> Combinación de perfiles</span> reglas creadas con <span class="wintitle"> Vínculo de perfil</span> son ideales para: </p> <p> 
     <ul id="ul_FF44FA894BB2448887C8EDA9C8407EF9"> 
      ...
      <li id="li_BE7112EE611E4DEB95B5C0A2852BFA97">Campañas específicas de bajo alcance. El <span class="wintitle"> Vínculo de perfil</span> el gráfico del dispositivo se basa únicamente en datos deterministas. Este grupo de perfiles de dispositivos siempre será más pequeño en relación con el grupo de usuarios y dispositivos no autenticados. </li> 
