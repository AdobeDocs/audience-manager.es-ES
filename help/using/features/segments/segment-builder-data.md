...
Agregar y quitar [!UICONTROL traits] in [!UICONTROL Segment Builder] para ver los datos reales [!UICONTROL trait] junto con los datos de población de segmentos reales y estimados. Los datos del tamaño estimado de la población le ayudan a crear el segmento adecuado para su campaña.

## [!UICONTROL Trait] Datos de población {#trait-population-data}

[!UICONTROL Segment Builder] muestra [!UICONTROL Total Trait Population] para el último día en que se agrega un [!UICONTROL trait] a un segmento. Estos datos aparecen en el campo azul alrededor del campo seleccionado [!UICONTROL trait] en el [!UICONTROL Basic View] sección.
...
Entrada [!UICONTROL Segment Builder], una barra azul en los gráficos de población estimada indica los posibles intervalos superior e inferior para el tamaño del segmento. Aunque el rendimiento anterior no garantiza resultados futuros, los datos estimados pueden ayudarle a comprender el tamaño potencial de un segmento nuevo o editado.
...
* Para datos de población de segmentos estimados (tiempo real y total), [!UICONTROL Segment Builder] no actualiza los gráficos automáticamente a medida que se añaden o eliminan rasgos de un segmento. Clic **[!UICONTROL Calculate Estimates]** para ver (o actualizar) los números de población estimados.

* Para datos reales (reales) de población de segmentos (tiempo real y total), [!UICONTROL Segment Builder] actualiza el gráfico de segmentos automáticamente al cargar un segmento existente. Para los segmentos nuevos o cuando se añaden nuevas características a un segmento existente, los datos de población reales no se actualizan hasta 24 horas después de la creación del segmento.
...
<table id="table_B24503F372E34B6BBDF5204181701A59"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>El número estimado de visitantes únicos vistos en tiempo real durante el intervalo de tiempo especificado y que estaban cualificados para el segmento en el momento en que fueron vistos por el Audience Manager. </p> <p>Entrada <span class="wintitle"> Generador de segmentos</span>, las poblaciones de los últimos 30 días para rasgos (<span class="wintitle"> Total de poblaciones de rasgos</span>), puede ser diferente para rasgos y segmentos evaluados en tiempo real. </p> <p>
     ...
     </ul> </p> <p> <p>Nota: La <span class="wintitle"> Población estimada en tiempo real</span> La métrica no incluye dispositivos que se han clasificado para un segmento basado en conexiones proporcionadas por un <span class="wintitle"> Regla de combinación de perfiles</span> que utiliza un <a href="../../features/profile-merge-rules/merge-rule-definitions.md#device-options"> opción de gráfico del dispositivo</a>. </p> </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>El número estimado de visitantes únicos que podrían estar en el segmento nuevo o modificado. Como con casi cualquier estimación, el rendimiento anterior no garantiza resultados futuros, pero puede utilizar el total estimado para lo siguiente: </p> <p> 
     ...
     </ul> </p> <p> <p>Nota: La <span class="wintitle"> Población total estimada</span> La métrica no incluye dispositivos que se han clasificado para un segmento basado en conexiones proporcionadas por un <span class="wintitle"> Regla de combinación de perfiles</span> que utiliza un <a href="../../features/profile-merge-rules/merge-rule-definitions.md#device-options"> opción de gráfico del dispositivo</a>. </p> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
[!UICONTROL Profile Merge Rules] afectan a los números de población reales y totales. Estos totales varían en función de si la variable [!UICONTROL Profile Merge Rule] un segmento a utiliza una opción de gráfico del dispositivo o no. Consulte también. [Opciones definidas de reglas de combinación de perfiles](../../features/profile-merge-rules/merge-rule-definitions.md).

### Datos de población de segmentos para [!UICONTROL Merge Rules] Sin un [!UICONTROL Device Graph Option]

La siguiente tabla define las métricas de población reales en tiempo real y totales cuando un usuario utiliza sus segmentos [!UICONTROL Profile Merge Rule] creado sin un [!UICONTROL device graph] opción. Estos son los ajustes de las opciones del dispositivo **[!UICONTROL No Device Options]** y **[!UICONTROL Current Device Proflie]**.
...
### Datos de población de segmentos para [!UICONTROL Merge Rules] Con un [!UICONTROL Device Graph] Opción

La siguiente tabla define las métricas de población reales en tiempo real y totales cuando un usuario utiliza sus segmentos [!UICONTROL Profile Merge Rule] creado con un [!DNL device graph] opción. Estas son las opciones de configuración del dispositivo para [!UICONTROL Profile Link Device Graph], el [!DNL Adobe] [!DNL device graph], y otros terceros [!DNL device graph] opciones disponibles para usted.
