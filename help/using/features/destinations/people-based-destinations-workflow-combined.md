---
description: 'Esta página incluye instrucciones paso a paso sobre cómo combinar datos de CRM sin conexión con datos de comportamiento que ya tiene en Audience Manager para crear nuevos segmentos de audiencia y luego enviar estos segmentos de audiencia a destinos basados en personas.  '
seo-description: 'Esta página incluye instrucciones paso a paso sobre cómo combinar datos de CRM sin conexión con datos de comportamiento que ya tiene en Audience Manager para crear nuevos segmentos de audiencia y luego enviar estos segmentos de audiencia a destinos basados en personas.   '
...
Esta página incluye instrucciones paso a paso sobre cómo combinar datos sin conexión [!DNL CRM] con datos de comportamiento que ya tiene en Audience Manager para crear nuevos segmentos de audiencia y luego enviar estos segmentos de audiencia a [!DNL People-Based Destinations].
...
>[!IMPORTANT]
>
> Este paso solo se aplica al [Escenario 2](people-based-destinations-workflow-combined.md#configure-data-source-settings) descrito anteriormente. Si los [DPUUIDs](../../reference/ids-in-aam.md) existentes ya son direcciones de correo electrónico con hash, vaya al [Paso 3 - Crear una regla de combinación de perfiles para la segmentación](people-based-destinations-workflow-combined.md#create-merge-rule).

Supongamos que desea hacer coincidir los [DPUUID](../../reference/ids-in-aam.md) existentes con las direcciones de correo electrónico con hash de la tabla siguiente (columna derecha) y almacenar las direcciones de correo electrónico con hash en la nueva fuente de datos que ha creado en [Paso 1: Configuración de fuentes de datos](people-based-destinations-workflow-combined.md#configure-data-source-settings).
...
El siguiente paso es crear una nueva regla de combinación que le ayudará a crear los segmentos de audiencia que desea enviar a los destinos basados en personas.

>[!IMPORTANT]
>
> Si ya tiene una regla definida con las opciones [!UICONTROL Current Authenticated Profiles] o [!UICONTROL Last Authenticated Profiles] , puede ir al [Paso 4 - Crear segmentos de audiencia](people-based-destinations-workflow-combined.md#create-audience-segments).
...
1. En la lista **[!UICONTROL Cross-Device Profile Options]**, seleccione las fuentes de datos en las que desea ejecutar la segmentación. Estas deben ser las fuentes de datos que contengan sus [DPUUID](../../reference/ids-in-aam.md) existentes.
...
>[!IMPORTANT]
>
>Un Audience Manager gestiona la integración con plataformas sociales mediante tokens de autenticación que caducan después de un cierto tiempo. Consulte Renovación de tokens de autenticación para obtener más información sobre cómo renovar los tokens caducados.
...
1. En la sección **[!UICONTROL Segment Mappings]** , seleccione los segmentos que desee enviar a este destino. Estos serían los segmentos que ha creado en [Paso 4: Crear segmentos de audiencia](people-based-destinations-workflow-combined.md#create-audience-segments).
1. Guarde el destino.
