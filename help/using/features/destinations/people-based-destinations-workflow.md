...
**B) Segmentación de audiencia basada exclusivamente en su actividad de usuario sin conexión**. En este escenario, el sistema [!DNL CRM] contiene las direcciones de correo electrónico de los clientes y otros atributos del cliente, pero los clientes no han interactuado con el sitio web, por lo que no hay ninguna actividad del cliente en Audience Manager. Este es un ejemplo que ilustra este escenario:
...
Dependiendo del tipo de ID de cliente que tenga en Audience Manager (consulte [3. Identifique el tipo de ID de cliente (CRM ID) que tiene ](people-based-destinations-workflow.md#identify-customer-id), en uno de los siguientes casos:

**A) Etiquete un origen de datos existente**. Esta opción se aplica al escenario en el que los ID de cliente de Audience Manager ([DPUUID](../../reference/ids-in-aam.md)) ya sean direcciones de correo electrónico con hash y minúsculas. En este caso, lo que debe hacer es etiquetar el origen de datos en el que almacena los ID como origen de datos de [!DNL PII]. Consulte [Configuración de Data Source](../datasources-list-and-settings.md) para obtener más información sobre la configuración del origen de datos. Lo que debe hacer es asegurarse de que la opción No se puede asociar a información de identificación personal esté desmarcada.
