...
Enumera y describe la sintaxis y los atributos admitidos (o pares clave-valor) que puede pasar al [!UICONTROL Data Collection Servers] ([!DNL DCS]). Esta información puede ayudarle a dar formato a su [!DNL DCS] y comprenda los parámetros devueltos por este sistema.

## Prefijos de atributo {#attribute-prefixes}

El [!DNL DCS] se basa en prefijos específicos agregados a las claves en pares de clave-valor para clasificar el tipo de datos que está pasando.
...
Todos son opcionales, a menos que desee una respuesta del [!DNL DCS]. Si desea el [!DNL DCS] para devolver una respuesta, haga lo siguiente `d_rtbd=json` es obligatorio.
...
<table id="table_FCCE4F9D796648899772A191981EFDE6"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Se utiliza para identificar al llamador que realiza la llamada al <span class="wintitle"> DCS</span> API. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p><code> d_cb</code> </p> </td> 
   <td colname="col2"> <p>Especifica una función JavaScript que desea ejecutar mediante <span class="wintitle"> DCS</span> Respuesta como parámetro de función de la función de llamada de retorno. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p><code> d_cid</code> </p> </td> 
   <td colname="col2"> <p>Contiene uno o más pares de ID de proveedor de datos (<code> DPID</code>) e ID de usuario del proveedor de datos (<code> DPUUID</code>) asignado por <span class="keyword"> Audience Manager</span>. Si utiliza varios pares de <code> DPID</code>s y <code> DPUUID</code>s, separe cada par con el carácter no imprimible <code> %01</code>. Por ejemplo: <code><i>DPID</i>%01<i>DPUUUID</i></code>. </p> <p><code> d_cid</code> reemplaza <code> d_dpid</code> y <code> d_dpuuid</code>, que están en desuso pero siguen siendo compatibles. Consulte <a href="../../../reference/cid.md">CID reemplaza DPID y DPUUID</a>. </p> </td>
  </tr>
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Deshabilite el uso de cookies de terceros para cumplir con las regulaciones de protección infantil. Este parámetro lo establece de forma dinámica el servicio de identidad de Adobe Experience Platform de Adobe y depende de la variable <code> idSyncDisable3rdPartySyncing</code> configuración. Consulte <a href="https://experienceleague.adobe.com/docs/id-service/using/reference/coppa.html" format="https" scope="external"> Compatibilidad con COPPA en el servicio de identidad de Adobe Experience Platform</a>. </p> </td>
  </tr>
  <tr> 
   <td colname="col1"> <p><code> d_cts=1</code> </p> <p><code> d_cts=2</code> </p> </td> 
   <td colname="col2"> <p>Opcional. Activado a petición del cliente. Póngase en contacto con su asesor de Adobe Audience Manager o con el Servicio de atención al cliente. </p> <p>Indica que los rasgos y segmentos deben devolverse dentro del <code> JSON</code> respuesta. </p> <p> 
     <ul id="ul_8B936ACB18724681B959783421ACF026"> 
      <li id="li_792A6248F49141C0B4B214C754D5F5C5"> <p><code> d_cts=1</code> devoluciones <a href="../../../reference/ids-in-aam.md"> ID de segmentos heredados</a> para los segmentos. </p> </li>
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Devuelve los datos de destino de la URL en <code> JSON</code> respuesta. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Indica el <code> JSON</code> versión que se utilizará en la respuesta. Normalmente, debe establecer esto en <code> d_jsonv=1</code>. Configuración <code> d_jsonv=0</code> deshabilita la sincronización de ID. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p><code> d_mid</code> </p> </td> 
   <td colname="col2"> <p>Especifica el ID de Experience Cloud establecido y utilizado por <span class="keyword"> Experience Cloud</span> Servicio de ID. Para obtener más información sobre el ECID, consulte <a href="https://experienceleague.adobe.com/docs/id-service/using/intro/cookies.html" format="https" scope="external"> Cookies y el servicio de identidad del Experience Cloud</a>. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p><code> d_nsid</code> </p> </td> 
   <td colname="col2"> <p>Identificador de espacio de nombre. Indica qué contenedor de JavaScript se utiliza. Utilizado por <span class="wintitle"> DIL</span> a para la sincronización de id. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>El propósito es idéntico al de <code> d_tdpid</code> Parámetro descrito anteriormente. Sin embargo, en este caso, la fuente de datos se pasa mediante el código de integración. </p> <p>Para mantener los rasgos descritos anteriormente, considere la llamada de muestra: </p> <p>Para <code>yourcompany.demdex.net/event?key1=val1&amp;key2=val2&amp;d_tdpid_ic=ic2</code>, solo se devuelve el rasgo T2. </p> </td> 
  </tr> 
  <tr> 
...
| `h_user-agent` | Configure como. `User-Agent` valor del encabezado. |
| `h_accept-language` | Configure como.  `Accept-Language`  valor del encabezado. |
| `h_referer` | Configure como. `Referer` valor del encabezado. |
| `h_referrer` | Configure como. `Referrer` valor del encabezado. |
| `h_date` | Configure como. `Date` valor del encabezado. |