...
Un informe de [!UICONTROL General] devuelve datos de rendimiento sobre atributos, segmentos y destinos.
...
1. Configure las siguientes opciones:
   ...
   * **De fechas a:** Especifique el intervalo de fechas para el informe.

2. Busque un rasgo, segmento o destino por nombre o ID.
3. En la lista de carpetas, arrastre y suelte los atributos, segmentos o destinos sobre los que desea informar en el panel [!UICONTROL Selections] de la derecha.
...
1. En la lista desplegable **[!UICONTROL Report Type]**, seleccione el tipo deseado: Trait, Segment o Destination.
...
* Estos números no incluyen los ID de visitante con tráfico excesivo. El tráfico de los bots se filtra antes de llegar a nuestro sistema de backend. Además, parte del tráfico de bots se descarta durante una tarea de limpieza semanal ejecutada en el back-end.
* Si los datos incorporados mediante procesamiento entrante eliminaron el UUID [!DNL Audience Manager] y estos ID incluyen usuarios que ya no están activos en nuestro sistema, estos UUID [!DNL Audience Manager] inactivos nunca llegan a [!UICONTROL User Profile Store] y no se notifican.
* [!UICONTROL Total Trait Realizations] se calculan solo para [!UICONTROL Rule-based Traits].
...
* [!UICONTROL Unique Trait Realizations] es el número de visitantes de dispositivos anónimos que han agregado el rasgo a su perfil dentro del intervalo de tiempo seleccionado.
* [!UICONTROL Total Trait Realization] es el número total de realizaciones de rasgos anónimos dentro del intervalo de tiempo seleccionado.
* [!UICONTROL Total Trait Population] es el número de visitantes de dispositivos anónimos que tienen este rasgo en su perfil.
...
Esta métrica representa el número real de visitantes únicos vistos en tiempo real para el intervalo de tiempo especificado y que estaban cualificados para el segmento en el momento en que los vio el Audience Manager.

### Población total de segmentos

Esta métrica representa el número total de UUID de Audience Manager cualificados para el segmento dentro del período de revisión seleccionado. Tu población total de segmentos de 1 día representa tu base de usuarios más precisa para la segmentación.
